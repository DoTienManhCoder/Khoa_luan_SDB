...
def render_paper_tex_macros(manifest: dict[str, Any]) -> str:
    ...
    for dataset, short in (("shot", "Shot"), ("bbc", "BBC"), ("clipshots", "Clip")):
        ...
        definitions[f"PaperSeed{short}FOneMean"] = f4(block["mean"]["f1"])
        definitions[f"PaperSeed{short}FOneStd"] = f"{block['sample_std']['f1']:.4f}"
    lines = ["% Generated by scripts/sync_experimental_results.py. Do not edit."]
    lines.extend(rf"\newcommand{{\{name}}}{{{value}}}" for name, value in definitions.items())
    lines.append("")
    return "\n".join(lines)
...
def render_paper_tex_tables(manifest: dict[str, Any]) -> str:
    ...
    seed_study = manifest["supplemental_results"]["journal_seed_study"]
    seed_labels = {"shot": "SHOT", "bbc": "BBC", "clipshots": "ClipShots"}
    seed_rows = []
    for dataset, label in seed_labels.items():
        block = seed_study["datasets"][dataset]
        mean = block["mean"]
        std = block["sample_std"]
        seed_rows.append(
            f"{label} & "
            f"{mean['f1']:.4f} $\\pm$ {std['f1']:.4f} & "
            f"{mean['precision']:.4f} $\\pm$ {std['precision']:.4f} & "
            f"{mean['recall']:.4f} $\\pm$ {std['recall']:.4f} \\\\"
        )
    lines += [r"\newcommand{\PaperSeedRobustnessRows}{%", *seed_rows, "}", ""]
    return "\n".join(lines)
