...
Phụ lục này tổng hợp lại các kết quả F1 chính trên hai bộ dữ liệu được dùng để phân tích sâu trong Chương~\ref{ch:ket_qua_thuc_nghiem}: SHOT và ClipShots. Các bảng chỉ giữ những số liệu đã được dùng trong phần thực nghiệm chính, tránh đưa vào các dòng chi tiết chưa có dữ liệu đối chiếu đầy đủ.
...
Mức suy giảm khoảng $3.1$ điểm phần trăm của AutoShotV2 so với AutoShot theo báo cáo gốc trên ClipShots cho thấy cấu hình tối ưu cho SHOT chưa chuyển dịch ổn định sang miền dữ liệu đa dạng hơn. Phân tích sâu hơn (xem Chương~\ref{ch:ket_qua_thuc_nghiem}, Mục Phân tích lỗi và khả năng tổng quát hóa) chỉ ra ba giả thuyết kỹ thuật chính:
\begin{enumerate}
    \item Nhãn many-hot $\pm 1$ frame chưa phù hợp với chuyển cảnh hard-cut sắc nét đặc trưng của ClipShots.
    \item Hệ số temperature scaling $T^*$ được tối ưu trên validation SHOT bị lệch khi áp dụng sang ClipShots.
    \item Cấu hình Focal Loss ($\gamma = 1.0, \alpha = 0.5$) thiên về phân phối nhãn của SHOT.
\end{enumerate}
...
Bảng~\ref{tab:all_experiments_master} tổng hợp tất cả các dòng thực nghiệm đã được sử dụng để thảo luận trong Chương~\ref{ch:ket_qua_thuc_nghiem}. Các giá trị trong ba cột bộ dữ liệu là F1-score. Do các nhóm thí nghiệm dùng protocol khác nhau, bảng này dùng để tra cứu tổng quan; việc so sánh kết luận nên dựa trên từng nhóm riêng trong Chương~\ref{ch:ket_qua_thuc_nghiem}.
