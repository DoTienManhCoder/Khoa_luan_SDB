...
\textbf{Định nghĩa hình thức.} Đầu vào của bài toán là một video $V = \{f_1, f_2, \dots, f_n\}$, trong đó $f_i$ là khung hình thứ $i$. Đầu ra là tập các vị trí ranh giới $B = \{b_1, b_2, \dots, b_m\}$, với $b_j$ là chỉ số khung hình bắt đầu một shot mới.
...
\begin{itemize}
    ...
    \item \textbf{Thiếu dữ liệu chuẩn cho video ngắn:} Các bộ dữ liệu truyền thống như BBC \cite{baraldi2015rai} hay RAI chủ yếu chứa phim tài liệu hoặc tin tức với cảnh quay tương đối tĩnh, không phản ánh được độ phức tạp của video ngắn hiện đại.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{RQ2:} Các kỹ thuật huấn luyện và hậu xử lý ở tầng phân loại (Focal Loss, many-hot labels, temperature scaling, Gaussian smoothing) có thể cải thiện F1 mà không cần thay đổi mạng xương sống hay không?
...
\begin{itemize}
    ...
    \item \textbf{Đóng góp về phương pháp:} Phân tích và triển khai quy trình AutoShot/AutoShotV2 \cite{zhu2023autoshot,soucek2020transnetv2}, trong đó bổ sung nhóm kỹ thuật cải tiến ở tầng phân loại và hậu xử lý (Focal Loss, nhãn many-hot, hiệu chỉnh nhiệt độ, làm mượt Gaussian) mà không thay đổi mạng xương sống gốc.
    \item \textbf{Đóng góp về kết quả thực nghiệm:} Trên tập SHOT, AutoShotV2 đạt F1 = 0.8545 ($+5.5$ điểm phần trăm so với mô hình cơ sở TransNetV2); đồng thời cung cấp phân tích so sánh chéo trên BBC và ClipShots để đánh giá khả năng tổng quát hóa (chi tiết xem Chương~\ref{ch:ket_qua_thuc_nghiem}).
\end{itemize}

\section{Tổ chức khóa luận}
Báo cáo được cấu trúc thành 6 chương:
\begin{itemize}
    ...
    \item \textbf{Chương 4 -- Bộ dữ liệu và thiết lập thực nghiệm:} Mô tả các bộ dữ liệu (SHOT, BBC, RAI, ClipShots), chỉ số đánh giá, mô hình cơ sở, siêu tham số và kịch bản thực nghiệm.
    \item \textbf{Chương 5 -- Kết quả thực nghiệm và thảo luận:} Trình bày kết quả so sánh, phân tích Precision/Recall/F1, thực nghiệm phân rã, phân tích lỗi, khả năng tổng quát hóa và trả lời câu hỏi nghiên cứu.
    \item \textbf{Chương 6 -- Kết luận và hướng phát triển:} Tổng kết kết quả chính, nhắc lại đóng góp, nêu hạn chế và đề xuất các hướng phát triển tiếp theo.
\end{itemize}
