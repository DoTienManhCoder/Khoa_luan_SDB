...
\node[block, fill=orange!15, right=of feat] (head) {Calibrated\\classification head};
...
frozen; AutoShotV2 retrains the decision stage and adds calibration and
temporal smoothing before thresholding. The deployed checkpoint uses
$T^{*}=\PaperDeployTemperature$; the controlled B4 replication of
Section~\ref{sec:protocol} selects $T=\PaperBFourTemperature$ under its
...
\begin{equation}
    ...
    \label{eq:branches}
\end{equation}
The primary branch $z^{(1)}_t$ is trained against the exact one-hot boundary
labels and is the only branch used at inference. The auxiliary branch
...
weight prevents the dense targets from blurring the primary branch's
localization.
...
\begin{equation}
    ...
    \label{eq:gaussian}
\end{equation}
Two operating points of this pipeline appear in the paper: the deployed
checkpoint fits $T^{*}=\PaperDeployTemperature$ on its validation pool,
while the controlled B4 replication selects $T=\PaperBFourTemperature$
...
all validation videos. This procedure selects $T=\PaperBFourTemperature$,
$\sigma=\PaperGaussianSigma$, and $\theta=\PaperDeployThreshold$. The
manifest containing folds, key hashes, search space, and selected values is
written before test evaluation. We denote the resulting BCE plus temperature
plus Gaussian configuration B4.
