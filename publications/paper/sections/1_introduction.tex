...
With a fixed deployment configuration, AutoShotV2 achieves an F1 score of \PaperDeployShotFOne{} on the SHOT test set, corresponding to an improvement of \PaperDeployVsAutoShotPP{} percentage points over the reproduced AutoShot baseline. For comparison, a supplemental checkpoint evaluated with oracle per-dataset thresholds reaches an F1 score of 0.8607 on SHOT (see Section~\ref{sec:oracle_sweep}). A controlled replication under a frozen,
validation-only
...
Focal Loss and boundary-aware many-hot supervision, and show that most of the
performance gain is attributable to calibrated post-processing, while
alternative training losses do not add consistent gains.
...
This paper employs two complementary evaluation tracks. The deployed checkpoint
