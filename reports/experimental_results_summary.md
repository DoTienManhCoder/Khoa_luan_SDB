...
| Ablation no-EMA | `A0_autoshot_original` - A0 -- AutoShot gốc | 0.8405 | 0.9554 | 0.7649 | AutoShot gốc, best threshold từng dataset. |
| Ablation no-EMA | `A1_phase2_bce_onehot` - A1 -- BCE + one-hot | 0.8378 | 0.9570 | 0.6983 | Control Phase2 tối giản. |
| Ablation no-EMA | `A2_focal_only` - A2 -- Focal only | 0.8378 | 0.9567 | 0.6967 | Chỉ thêm Focal Loss. |
| Ablation no-EMA | `A3_manyhot_only` - A3 -- Many-hot only | 0.8375 | 0.9563 | 0.7005 | Chỉ thêm nhãn many-hot. |
| Ablation no-EMA | `P1_gaussian_only` - P1 -- Gaussian only | 0.8432 | 0.9436 | 0.7519 | Tăng ClipShots nhưng giảm BBC. |
| Ablation no-EMA | `P2_temperature_only` - P2 -- Temperature only | 0.8378 | 0.9570 | 0.6983 | Gần như trùng control. |
| Ablation no-EMA | `B1_focal_manyhot` - B1 -- Focal + many-hot | 0.8384 | 0.9559 | 0.7006 | Tác động train-time nhỏ. |
| Ablation no-EMA | `B4_temperature_gaussian` - B4 -- Temperature + Gaussian | 0.8540 | 0.9570 | 0.7441 | Cấu hình hậu xử lý được ưu tiên. |
| Ablation no-EMA | `B5_full_candidate` - B5 -- Full candidate no-EMA | 0.8542 | 0.9551 | 0.7409 | Focal + many-hot + hậu xử lý. |
...
| EMA full fine-tune | `ema_phase1_raw` - A -- Phase 1 raw | 0.8412 | 0.9554 | 0.7648 | Baseline raw trong EMA study. |
| EMA full fine-tune | `ema_phase1_gaussian` - B -- Phase 1 + Gaussian | 0.8477 | 0.9515 | 0.7899 | Gaussian hậu xử lý rất mạnh trên ClipShots. |
| EMA full fine-tune | `ema_full_model_alpha999` - D-EMA, alpha=0.999 | 0.8417 | 0.9604 | 0.7860 | Fine-tune toàn model + EMA. |
| EMA full fine-tune | `ema_full_model_noema` - D-noEMA control | 0.8371 | 0.9494 | 0.7638 | Fine-tune toàn model, tắt EMA. |
